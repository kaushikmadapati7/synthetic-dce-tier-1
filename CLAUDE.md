...
**Re-evaluating a trained model (`--eval-only`).** Pass `--eval-only` (or `EVAL_ONLY=1` in the SLURM script) to skip training: `main` dispatches to the `LOADERS` registry (parallel to `TRAINERS`) which rebuilds the model, loads `<output-dir>/checkpoints/{model}_last.pt`, reloads the saved `harmonizer.json` (no re-fit), and runs `evaluate`. The LDM `scaling_factor` is **not** in the checkpoint (a plain attribute), so it is recomputed from the train latents exactly as in training. Writes `config_eval.json` (not clobbering `config.json`) and overwrites `metrics.json`. Primary use: get FID after a training run where the Inception download failed (set `TORCH_HOME` to a writable, non-quota'd path — the SLURM script defaults it to `$PWD/.torch`).
