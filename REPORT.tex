...
\emph{Sections 2--5 are deliberately shared across all three tracks} --- the cohort,
preprocessing, objective and metrics are common to every experiment, and duplicating them
per track would let the copies drift. What is track-specific is the evidence, so the results
below are labelled by which claim they support (see \S9 for the claims themselves, and
Appendix~A for which runs belong to which). Track~3 has no results yet; its readiness is
\S8.

\subsection{Track 1 --- single-phase synthesis: model comparison}
...
\flag{The 2D ranking reverses under pre-contrast conditioning.} With the fourth channel the
2D pixel-space flow reaches $\texttt{roi\_pearson}=0.439$ against the 2D GAN's $0.293$ ---
the opposite of every prior comparison --- and the 3D flow gains more than the 3D GAN
($+0.097$ vs $+0.058$; PZ $+0.148$ vs $+0.112$), halving the 3D gap from $0.113$ to $0.066$.
This is consistent with the flow's characteristic failure having been amplitude
(over-dispersion, and worse than a constant predictor on MAE), which is exactly what
pre-contrast repairs. Both 2D numbers are $n=1$ and there is no 2D noise floor yet, so the
reversal is not established --- but ``the GAN wins'' can no longer be stated as a general
result.
...
\subsection{Track 2 --- identifiability: what the model has actually learned}
\label{sec:decomp}
...
\subsection{Tracks 1 and 2 --- pre-contrast conditioning}
...
\paragraph{Seed repeats.} Four runs at seeds $0$--$3$ with the split held fixed
(\texttt{-{}-split-seed 0}), against $n=2$ without pre-contrast:

\begin{center}
\begin{tabular}{lcc}
\toprule
& no pre-contrast ($n{=}2$) & $+$ pre-contrast ($n{=}4$) \\
\midrule
\texttt{roi\_pearson} & 0.482,\,0.466 & $\mathbf{0.532\pm0.024}$ \\
\texttt{ssim\_roi} & 0.397,\,0.389 & $0.453\pm0.018$ \\
PZ \texttt{roi\_pearson} & 0.339,\,0.330 & $\mathbf{0.447\pm0.024}$ \\
\texttt{p75\_corr} & 0.357,\,0.081 & $0.748\pm0.028$ \\
\bottomrule
\end{tabular}
\end{center}

The groups do not overlap: the \emph{worst} pre-contrast run ($0.500$) exceeds the
\emph{best} run without ($0.482$). PZ separates by $0.112$, $\approx4.7\times$ the standard
deviation. The measured $\mathrm{sd}=0.024$ also revises the earlier noise estimate upward
from $0.017$, which was extrapolated from a single replicate pair.
...
\paragraph{A prediction that held: instability is a property of the target, not the metric.}
\texttt{p75\_corr} swung $0.276$ between two \emph{identical} runs without pre-contrast.
Across four pre-contrast runs it is $\pm0.028$ --- a tenfold reduction, same metric, same
code, same cohort. The only change is that the quantity it measures became predictable.
This is the sharpest available evidence for the claim below: the metric was unstable
\emph{because} its target was not determined by the inputs, and supplying the missing input
removed the instability. It was predicted before the pre-contrast run and is therefore a
falsifiable test rather than a post-hoc reading.

\paragraph{Why \texttt{p75\_corr} was $16\times$ worse.} \texttt{roi\_pearson} averages $N=550$
...
Which runs support which claim. Runs before \texttt{v3\_*} used the pixel-count crop
(\S3.1) and are superseded; runs before the 2026-07 audit are superseded entirely.
All 3D runs share one split (\texttt{split\_seed 0}, 550 test cases).

\begin{center}
\small
\begin{tabular}{p{4.5cm}p{1.1cm}p{1.4cm}p{1.0cm}p{5.0cm}}
\toprule
Run group & Track & Model & $n$ & Purpose / status \\
\midrule
\texttt{v3\_3d\_gan} & 1 & GAN & 1 & iso-framing baseline; superseded as headline \\
\texttt{v3\_3d\_flow\_medvae} & 1 & flow & 1 & GAN-vs-flow, no pre-contrast \\
\texttt{v3\_2d\_*} (4 runs) & 1 & both & 1 ea. & 2D track incl.\ the iso A/B (inside noise) \\
\midrule
\texttt{v4\_3d\_gan\_pregad} & 2 & GAN & 1 & \emph{accidental} no-pregad duplicate of v3; the nondeterminism replicate \\
\texttt{v4b\_3d\_gan\_pregad} & 1,2 & GAN & 1 & first pre-contrast run \\
\texttt{v5\_3d\_gan\_pregad\_s\{1,2,3\}} & 1,2 & GAN & 3 & seed repeats $\to$ $0.532\pm0.024$ \\
\texttt{v5\_3d\_flow\_medvae\_pregad} & 1 & flow & 1 & flow $+$ pre-contrast \\
\texttt{v5\_2d\_\{gan,flow\_pixel\}\_pregad} & 1 & both & 1 ea. & where the 2D ordering reversed \\
\midrule
\texttt{v6\_3d\_gan\_nopregad\_s\{1..3\}} & 1,2 & GAN & 3 & completes 4-vs-4 for the pre-contrast claim \\
\texttt{v6\_2d\_\{gan,flowpix\}\_pregad\_s\{1..3\}} & 1 & both & 3 ea. & settles the 2D reversal \\
\texttt{v6\_3d\_flow\_\{no,\}pregad\_s\{1..3\}} & 1 & flow & 3 ea. & error bars on the flow arms \\
\texttt{v6\_3d\_flow\_pregadsrc} & 3 & flow & 1 & ODE integrates pre $\to$ post; the two-timepoint case of \S8 \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Naming.} \texttt{v}$k$ marks a pipeline generation, not a model version:
\texttt{v3} = iso framing $+$ checkerboard fix, \texttt{v4} = pre-contrast, \texttt{v5} =
first seed repeats, \texttt{v6} = matched-$n$ completion. Comparisons \emph{within} a
generation are safe; comparisons across them must be checked against \S7.1, since a
generation boundary usually means a defect was fixed in between.
