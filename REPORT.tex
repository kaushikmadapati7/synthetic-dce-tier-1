...
predictable, per-patient enhancement \emph{amplitude} is not, and amplitude carries $51\%$
of the signal. Supplying the pre-contrast phase as a fourth input channel improves every
metric substantially --- \flag{pending a passthrough control that is not yet run}. Methods
and the measurements that justify them are presented together throughout.
\end{abstract}
...
\flag{This result is not yet interpretable, and should not be presented as settled.}
Pre-contrast is a far stronger predictor of post-contrast than T2w is --- same acquisition,
same receiver gain, and post $\approx$ pre $+$ enhancement. A model handed that channel can
depress MAE by largely copying it, and a $43\%$ global MAE drop is precisely what copying
would produce. The \texttt{t2w copy} baseline established that the 3-channel model was not
laundering its input; there is no equivalent control for the fourth channel yet. Further,
$\texttt{p75\_corr}$ is the noisiest metric available ($\Delta=0.276$ between
\emph{identical} runs, \S\ref{sec:variance}), so a $+0.386$ change is only $\approx1.4\times$
noise and cannot carry the claim alone.

The decisive comparison is a \textbf{pre-contrast passthrough baseline}
($\yhat_v=x^{\text{pre}}_v$) plus the two stable amplitude readouts --- brightness tracking
and between-case predicted spread, both averaged over 550 cases. One encouraging signal
survives the objection: passthrough would lift PZ and TZ roughly together, whereas PZ gained
$31\%$ against TZ's $5\%$.
...
\S\ref{sec:decomp} plus the reproducibility-tracks-identifiability observation. Its framing
