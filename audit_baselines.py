...
and the signed bias, since a systematic offset inflates every ROI metric.
    ...
    python -m tier1_static.audit_baselines --model gan --ucsf-main-root <staged> \
        ...
        --dce-robust-k 1 --spatial-size 32 192 192 --split val
"""
...
def main(argv=None):
    ...
    for batch in loader:
        ...
        for i in range(pred.size(0)):
            ...
            rows.append({
                ...
                "mae_t2w": float((cond[i][0][m].float() - t).abs().mean()),
            })
    if not rows:
    ...
    print(f"{'predictor':10s} {'MAE_roi':>9s}   what it knows")
    print("-" * 58)
    print(f"{'const':10s} {mae_const:9.4f}   nothing (one number for every case)")
    print(f"{'t2w copy':10s} {A['mae_t2w'].mean():9.4f}   identity baseline")
    print(f"{'MODEL':10s} {A['mae_model'].mean():9.4f}   <-- trained generator")
